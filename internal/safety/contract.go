...
// Contract 是一个执行器 Resource 的 Safety Contract。它是 OEM 在
// Provider manifest 的 `safety_contract` 中声明、并由 nervud Policy 独立校验后确定的
// 时间预算与升级窗口。
//
// v1 只定义类型与校验；manifest 解析留到 service/Provider 接线时实现。
// 平台不规定统一停稳毫秒数：真实数值来自机器人风险分析、制动能力和
...
type Contract struct {
	...
	// 为 false 时停止进度封顶在 OUTPUT_DISABLED。
	StandstillConfirmationSupported bool
}

// DefaultContract 返回一组保守的 fail-closed 占位默认值。真机必须按测量结果覆盖。
//
// ProviderAcceptTimeout 取 100ms 与 示例的 requested_ack_deadline_ms 对齐；
...
//
// 不得低于 Resource 安全地板的比较需要 Resource 风险模型，留到 resource 落地时
