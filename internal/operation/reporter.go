...
// 进度与终态，nervud 校验转移合法性 + origin/epoch 绑定后落库（NRCP §11.1 /
// spec §6）。Provider 不拥有最终系统 Operation 状态，也不能在系统取消后继续
...
// 任何越权或非法的 code 一律 fail-closed 归一化为 INTERNAL（铁律 1/3，绝不静默
...
// reportTerminal 是三个终态回报的公共实现，落实"终态只写一次"（铁律 2）：
//
//   - 已是【同一】终态：幂等 no-op，返回 nil。
//   - 已是【不同】终态：终态不可覆盖，记审计并返回 ErrAlreadyTerminal
//     （并发的 Succeed/Fail/Cancelled 由 mu 决出唯一胜者，败者走这里）。
//   - 越过撤销边界/已过 deadline：晚到的 Provider 推进【绝不允许成功】，同步
//     收敛为 FAILED 并返回原因（见下方"抢占检查"，铁律 1/4）。
//   - 转移非法（如 RUNNING→CANCELLED）：记审计并返回 ErrIllegalTransition。
//   - 否则：CAS 写入终态（terminateLocked），返回 nil。
//
// 抢占检查是本函数的关键防线：SafetySupersede/deadline 收敛都在【后台】异步做，
