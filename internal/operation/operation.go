...
// 对外事件, 以及唯一权威的合法转移判定 canTransition. 状态机是本包的正确性
// 核心, 与并发或装配无关, 单独成文件便于单测穷举全部转移.
package operation
...
// State 是 Operation 的生命周期状态
//
// 零值 StateUnspecified 永远不是合法结论: 与所有 *_UNSPECIFIED=0 枚举同一个
// 理由, 漏填必须 fail-closed, 不能因为没赋值就默默落到某个真实状态.
type State uint8
...
// Terminal 报告 s 是否为终态. 终态之后不接受任何转移.
func (s State) Terminal() bool {
	return s == StateSucceeded || s == StateFailed || s == StateCancelled
}

// canTransition 是唯一权威的合法转移判定.
// 其余一律非法, 调用方拒绝并记审计.
//
// 唯一允许集合:
//
...
// CANCELLED 不允许 (取消必先经 CANCEL_REQUESTED,
...
// OriginBinding 是 operation 创建时绑定的受信类型信息
//
// 它让 SDK 重连后能恢复解码 typed progress/result/error: detail 的 Protobuf
// 类型由这份绑定决定, 不使用 google.protobuf.Any 或客户端提供的 type URL.
// Get/事件投影必须返回它.
...
type Operation struct {
	...
	// conn 是拥有本 operation 的调用方连接, 未导出: 外部快照看不到它,
	// 只用于 ReleaseByConn 的按连接收敛. 系统内部创建时为 nil.
	conn ConnHandle

	// providerConn 是执行方连接, 即 nervud 把这次调用 Dispatch 给了谁.
	//
...
// Event 是订阅通道上投影给 SDK 的一条事件
//
// 携带 Origin 绑定: SDK 重连后据此恢复解码 typed payload/result/error.
type Event struct {
	OperationID uint64
	Kind        EventKind
...
// 供审计离线分析与调用方分支.
var (
	// ErrNotFound 表示 id 不存在或 caller 不可见. 两种情况使用同一错误,
	// 避免泄露他人的 operation 是否存在.
	...
	// 之前, 已被系统接管为 FAILED; Provider 的任何后续推进都被同步拒绝.
