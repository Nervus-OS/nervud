...
// State 是顶层 Safety 状态机的状态（架构 §6 / NRCP §14.1）：
//
//	NORMAL → SAFETY_LATCHED → OEM_RECOVERY(可选) → REARM_REQUIRED → NORMAL
//
// 取值从 1 起，0 保留为 StateInvalid：使零值 Gate 不落在任何「可放行」态，
// 配合 epoch==0 无效构成双重 fail-closed（见包文档）。
type State uint8
...
// String 返回顶层状态名，与三份 remember/ 决策文档的顶层 Safety 状态名一致。
