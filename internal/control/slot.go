...
func (m *Module) issueLocked(req Request, now time.Time, ttl, deadman time.Duration) (Lease, error) {
	// ：先废止旧 Lease/队列，再以新 epoch 签发新 Lease
	m.cur.Store(nil)
...
// 见 lease.go 的 Owner 说明），不信任何自报值。v1 只有单个租约槽，所以最多
// 撤一条；API 形状按撤该包全部租约定义，v2 放开多 Resource / 多槽时调用方不用改。
//
...
func (m *Module) RevokeAll(epoch uint64) {
	...
	m.revLease[i].Store(old) // 后写 lease：消费者以它非 nil 为该格已就绪的标志
...
//
// 只读一次 Gate，把这次读到的 (st, ep) 同时用于来源判定与租约有效性复核。
...
func (m *Module) lookup(id ID, conn ConnID, now time.Time) (*Lease, error) {
	l := m.cur.Load()
	// 租约不能转让：ID 与连接必须同时对上（ owner_connection）
	if l == nil || l.ID != id || l.Conn != conn {
