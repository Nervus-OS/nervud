...
// 到 的映射（位置 A 是 Envelope 的通用 StatusCode，位置 B 是 typed detail）：
//
...
//
// ErrHeldByHuman 与 ErrHeldByAI 刻意分开而不是合成一个 BUSY：Agent 需要知道现在是
// 人在遥控才能去问用户要我接手吗 - 让出的决定权在人，而笼统的 BUSY 表达不了
...
var (
	...
	//
	// 超限即拒绝而不是静默压到上限：静默压短会让调用方以为自己有 5 秒、实际只有
...
var (
	...
	// 让离线规则能把连接断开被动失去控制权与因包生命周期/权限变更被撤租区分开
