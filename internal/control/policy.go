...
// 人会松手、会断网、会走开；Agent 不会「松手」，但会被模型或网络卡住
package control
...
// 人一松手、一断网就必须在几百毫秒内失去控制权并回到 NONE（Agent 文档 §6.1）。
//
// AI：TTL 1s，不要求 deadman。Agent 是本地进程，不会「松手」，所以 deadman 意义不大；
...
// 「只能更严」这条让 SDK 可以在弱网下主动缩短窗口，而系统安全地板不会被调松
