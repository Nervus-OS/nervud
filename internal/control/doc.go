...
// # 控制权模型
//
// ControlLease 从语义第一天起就是 Resource-scoped；[REWRITE-v1] 唯一合法 Scope 为
// {base.main}，因此内部退化为【单个租约槽】的 exclusive_control。
//
//	SAFETY > HUMAN > AI > NONE
//
// 这只是「有效控制来源」的产品层排序，且其含义仅限于执行器与运动控制权，不代表
// 全系统权限等级，也不代表 CPU 调度优先级（Agent 文档 §3.1）。四者不共用同一种
// owner 枚举或 Lease 实现：
//
//	SAFETY  独立 Gate/latch（internal/safety），不是普通 lease
//	HUMAN   普通 lease，Class = ClassHuman
//	AI      普通 lease，Class = ClassAI
//	NONE    nil lease —— 客户端不能申请它，它是「当前没有有效租约」的结论
//
// 并发不靠优先级维度解决，靠 Resource 维度：人遥控底盘时 AI 仍可控制其它 Resource。
// [REWRITE-v1] 只有 base.main 一个执行器 Resource，所以这一层暂时看不出来，但 API
// 形状从第一天就是 Resource-scoped，v2 放开时调用方不用改。
//
// # Class 由调用方裁决，本包不信任何自报值
//
// 远程手机在请求里写 source=HUMAN 一律不可信（Agent 文档 §6.2）。真正的裁决在 IPC
// 请求管线（身份 + Permission + Policy）完成，control 只在拿到【已裁决的】Class 之后
// 执行租约状态机。同理，「AI 现在该不该自主行动」由 Agent Service 的策略决定，本包
...
//
// ① RevokeAll 绝不取 mu。它由 safety 的 Supervisor Lane（FIFO 90）在 beginHalt 里调用；
// Go 的 mutex 没有优先级继承，等一个普通优先级的持锁者就是 FIFO 90 上的优先级反转。
// 它只做一次 atomic Swap，并把被撤的租约交给 Control Lane 事后记账。
//
// ② RevokeAll 不递增 epoch。NRCP §10.5 要求「每个边界只递增一次」，而 safety 触发时
// gate.Trip() 已经递增过了；这里再递增就是 churn。
//
// ③ 签发采用「发布后复核」：递增 epoch → 发布租约 → 重读 Gate；若此刻已锁存或 epoch
...
//
// 注意这与 Safety Stop Lane（FIFO 95）的零堆分配硬规则【不同】，不要互相套用：那条
// 规则针对的是急停投递路径，本 Lane 不在急停路径上。真正要求零分配的是 Check/Refresh
// ——它们在每条运动命令上被调用，由 AllocsPerRun 测试守住。
//
// # [REWRITE-v1] 已知取舍
//
//   - 内核层没有「机器人能不能自主动」的闸门：AI lease 的签发只受 Permission、Safety
