...
type Module struct {
	...
	// cur 是唯一的租约槽（[REWRITE-v1] 单 Resource base.main 的 exclusive_control）。
	// nil = NONE。读侧无锁；写侧除 RevokeAll 外都在 mu 保护下（见包文档并发规则）
	...
	// base 是单调时钟基准；fresh 记「最近一次新鲜输入」距 base 的纳秒。
	...
	// 由生命周期序列化，不并发。用「并行原子数组 + 环」而不是单指针槽：
	//   - 零堆分配：RevokeAll 只做原子写，绝不 new（由 alloc 测试守住）
	//   - 不丢事件：单槽在「撤销 A → rearm → 取 B → 撤销 B」快过一次 drain 时会丢 A
	...
	// laneWG 计 Control Lane 的在跑数；Stop 用它把「Lane 已退出」排在最终撤租/排空
...
//
// gate 必须是 main.go 里 motiongate.New() 出来的【同一个】实例——safety 与 control
// 共用它，epoch 不归任一方独占（见 internal/motiongate 包文档）
...
//
// SpawnDedicated 是同步握手：生产环境设不上 RT 优先级即返回 error → Start 返回
// error → 内核 fail-fast（README「生产环境实时优先级设置失败 = fatal」）
func (m *Module) Start(_ context.Context) error {
	...
	// SpawnDedicated 返回 nil ⟺ fn 一定会跑（scheduler 语义），Add/Done 恰好配对
	m.laneWG.Add(1)
...
//
// 停机后不留任何控制权：NRCP §10.5 要求新 Host session 从 NONE 开始、不恢复旧 Lease。
// 这里主动撤一次并递增 epoch，让「已经进入 Provider 队列的旧命令」在进程退出前就失效，
// 不依赖下次启动时 epoch 恰好不同。
//
// 顺序：先关 stopCh 让 Lane 退出，等它真正退出（waitLanes），再做最终排空与撤租。
// 这道序保证「Lane 已停」先于「Stop 消费撤销环」，二者不并发消费同一个环；也保证
...
//
// cause 非 nil 即置 Denied：让离线规则能一眼筛出「非正常结束的控制会话」（到期、
