// Package admin 是 nervud 的特权管理控制面：一条 root-only 的 UDS，让本机
...
//   - 管理操作是特权的（装包会把 staging 提交成系统里的可执行代码、改权限状态），
//     必须由系统持有的确认通道触发，而不是任何普通 App。授权权力只在 nervud；
//     危险动作只能请求，确认入口由系统持有。因此单开一条
//     0600、SO_PEERCRED 校验为运维身份的通道，两套准入规则不混。
//
// Package Registry 的唯一权威真源永远是
// nervud 进程内的 pkgregistry.Module。nervusctl不直接改磁盘 Registry - 它只
