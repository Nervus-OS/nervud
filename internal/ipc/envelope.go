...
// 这帧到底算不算一个合法的 Envelope，不回答此刻允不允许收到这种 body
package ipc
...
//
// 与 frame.go 的长度错误同级 - 那边是边界不可信，这边是边界可信但内容
...
//  2. body 必须已设置。空 Envelope 没有任何合法用途，容忍它等于给刷空帧
//     消耗预算留口子；来自未知新版本的未知 body 也会落到这里（proto3 把
//     不认识的 oneof 分支收进 unknown fields，Body 保持 nil），同样按违规处理
//
// 未知字段与未知body的处理刻意不同：前者被 proto 运行时保留并忽略，
// 满足的旧接收方可以忽略未知字段；后者意味着对端在用本端根本
...
// isProtocolViolation 判定一个错误是否属于这条连接已经没法继续
//
