// 本文件实现 Route（供 ipc 的 Request 分派查表）、UnregisterEndpoint 与
// ConnClosed（生命周期失效，设计方案 §5.3/§5.4）
package endpoint
...
// 架构 §10.5"解析 endpoint 时检查一次权限，每次调用时仍做快速权限与存活
...
// UnregisterEndpoint 撤下一个 Service 侧的 registration（设计方案 §5.4）
//
// req.Drain 在 v1 未接线：执行层没有在途 Dispatch 追踪，drain=true 退化为
// 立即失效，记录为已知简化（设计方案 §5.4 表格 / §9 Phase 2）
...
// 全部 registration/binding（设计方案 §5.4）
//
...
//     全部 endpoint 失效，重连必须重新 Resolve（架构 §10.5）
