// 本文件补齐 Package 的运行前置: 系统用户与私有数据目录.
//
// # 为什么单独一个文件
//
...
func (m *Module) provisionEntry(ctx context.Context, e Entry) error {
	...
	//
	// 已存在时 CreatePrivateDataDirectory 会以 EEXIST 失败, 那在这里是正常
	// 情况而不是错误: 本函数在每次启动扫描时对每个包都跑一遍.
	...
	// 包都能抢先创建别人的目录名 - 恶意包先建 nervus.camerad/ 并占为己有,
