// 本包把"这台机器上有哪些资源"包成一个内建 endpoint, 供 nervud 在装配期
...
// # 为什么由内核实现而不是某个系统服务
//
// 资源目录就是 Catalog 本身. 让一个服务来代答, 等于让它维护一份 Catalog
...
//
// handle / 类型 / role / 访问模式 / 风险级 / 标签. 不含设备节点路径,
...
func (m *Module) listResources(payload []byte) endpoint.BuiltinResult {
	...
	// 一次读定, 整份响应来自同一个 Snapshot. 分两次读会让响应里混进
...
//
//	(Provider PID, 设备节点) 时, 显式列举会让新字段默认不外泄, 而
//
