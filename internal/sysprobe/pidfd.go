...
// 本文件是 Component 核对（ipc.verifyComponent，应用层架构决策 §5.5）需要的两项
// 内核观察：SO_PEERPIDFD 拿对端进程的稳定引用（pidfd），以及从 PID 读 cgroup。
//
// 同属 sysprobe 的「纯观察」准入：只读，不改变任何系统状态、不持有 capability。
// 「cgroup 路径 → systemd unit → Component」这层解释是策略，放在 ipc，不在这里。
package sysprobe
...
//
// 相比 SO_PEERCRED 的裸 PID，pidfd 是对端进程的【稳定引用】：只要持有它，即便对端
// PID 被内核回收给别的进程，pidfd 仍唯一指向原进程（或明确报告其已退出），从根本上
// 免疫「PID 回收后核对到不相干进程」的竞态。调用方拿到 fd 后负责 Close。
//
...
// 二次校验：核对 /proc/<pid> 属主仍等于 SO_PEERCRED 的 UID（§5.5 回退项）
