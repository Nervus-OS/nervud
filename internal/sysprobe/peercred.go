...
// 新进程. 因此 PID 只是待核对的线索; Component ID 必须结合 PID, cgroup 和启动
// 记录交叉核对, 且核对必须在连接建立后尽快完成. 绝不允许在
// 连接存活很久之后再拿这个 PID 去 /proc 反查并信任结果
//
// UID 才是身份裁决的依据: 它稳定, 不受 PID 回收影响, 并且与 Package 一一对应.
// 调用方拿到 UID 后应先用 authority.Invariants.CheckUID 确认它落在
