// 本文件是 scheduler 的跨平台通用部分：Policy/优先级常量与 Lane 类型
// 包文档见 doc.go

package scheduler

// Policy 是与平台无关的调度策略抽象。具体到 Linux 时映射为 SCHED_FIFO / SCHED_RR /
// SCHED_OTHER；在非 Linux 平台上这些值只用于日志，不产生真实效果
type Policy int
...
const (
	...
	// 用于 Emergency Stop 与 Safety Supervisor 这类「一旦要跑就必须立刻跑完」的路径
	PolicyFIFO
...
// 这些数字表示「调度先后」，不表示 CPU 占用百分比，也不存在优先级 100
//
// 为什么上限取 95 而不是 99：
//
...
//
// 无论优先级高低，lane 一律不得自旋：优先级决定「谁先跑」，是否阻塞决定
...
// setSched 在已 LockOSThread 的当前线程上应用调度策略，由平台文件各自定义：
// sched_linux.go 是真实 syscall；sched_other.go 是 no-op
// build tag 保证任一平台恰好编译进一个实现——存在性由编译期保证，无需运行时注入
//
// （此处仅为文档锚点；函数体见平台文件）
...
// 按 sec 6 第 3/6 条，Stop Lane 热路径必须零堆分配，并有 benchmark 校验
type Lane struct {
	name     string
	policy   Policy
	priority int
...
// applySelf 在「当前 goroutine 已经 LockOSThread」的前提下，把本 Lane 的调度策略
