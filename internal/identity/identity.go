...
var (
	...
	//
	// 它必须与UID 越界区分开：越界是不变量违规（authority.CheckUID），
	...
	//
	// 这不是可以容忍的配置问题。如果让所有 Package 共用一个 UID，
	// SO_PEERCRED 将无法提供 Package 级隔离，
	...
	//
	// 与 ErrDuplicateUID 对称，Package 与 UID 必须一一对应。
	// 只查 UID 唯一挡不住同一个 Package ID 出现在两个 UID 下 - 那会让
...
// 原子指针，读侧完全无锁 - 避免出现装包正持写锁，所有新连接一起卡住
//
...
// 增量接口会诱使调用方做先删后加，而那两步之间存在一个 UID 查不到的窗口，
...
func (r *Registry) Replace(pkgs []Package) error {
	...
	for _, p := range pkgs {
		...
		case p.UID == 0:
			// UID 0 是 Linux 特殊 root 身份，App UID 0 永远禁止。
...
// 一律当作空索引返回 not-found，而不是 panic。装配错误应当表现为谁都不认识、
// 全部拒绝这种 fail-safe 状态，绝不能让首个连接把整个 accept 路径打崩 -
...
//
// 它只回答这个 UID 属于哪个 Package。UID 是否落在 App 区段内是
