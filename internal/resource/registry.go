...
// role/resource_handle/access_mode 是什么"（设计方案 §3/§4）
package resource

import "fmt"

// Entry 是一条 Resource 的权威定义（NRCP §8.2 字段的 v1 子集，设计方案 §3）
type Entry struct {
	Handle string // 运行期不透明句柄，v1 恒 "base.main"
	Type   string // 规范类型，如 "nervus.resource.motion.base"
	Role   string // 稳定角色，如 "main"

	// AccessMode 是 NRCP §8.5 定义的访问模式（如 "exclusive_control"）。
...
// endpoint.NewInterfaceCatalog 的自洽性校验（设计方案 §4.1）
...
//
// 不从任何 manifest 派生（设计方案 §5 决策 1）——与 permission.DefaultCatalog/
...
// （设计方案 §4.2）。只做精确匹配：type 对但 role 错、role 对但 type 错、两者
...
// 输入，因此空字符串天然不会出现在 byHandle 里，不需要额外特判（设计方案 §4.3）
