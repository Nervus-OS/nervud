...
// 业务路径。但它会拖慢【写 goroutine 自己】：SD 卡上一次 fsync 是几到几十毫秒，
...
// 丢 1 秒，而那 1 秒【会被下次启动的 UncleanShutdown 标出来】——窗口是已知的，
...
func (r *FileRecorder) open() error {
	...
	case last.Action != ActionClosed:
		...
		//
		// 链本身仍然连得上——正因为如此，少掉的那几条【看不出来】。这条记录
...
// 掉电时那 30 秒的记录整段消失，而剩下的链【完全自洽】——校验工具会报「通过」。
