...
//
// 校验跨文件: 轮转后的历史文件与当前文件是同一条链, 新文件的第一条带着
// 旧文件的末尾哈希. 逐个文件单独校验会在每个文件边界上误报.
//
// # 它不回答的问题
//
// "有没有人把整份审计重写一遍并重算全链". 回答这个问题需要远程日志, TPM
// 或一次性写入介质等外部锚点; 本地校验器不拥有这些锚点, 因此不会声称覆盖它们.
func cmdAuditVerify(args []string, stdout io.Writer) error {
	...
	// prev/seq 跨文件串下去, 因此必须从最旧的文件开始.
	//
	// 起点不能假设是 seq 1: 轮转的保留策略会丢弃最老的文件, 稳态下最旧的
